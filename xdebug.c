...
PHP_MINIT_FUNCTION(xdebug)
{
	...
	if (XG(coverage_enable)) {
		...
		XDEBUG_SET_OPCODE_OVERRIDE_COMMON(ZEND_BOOL_AND);
		XDEBUG_SET_OPCODE_OVERRIDE_COMMON(ZEND_BOOL_OR);
#if PHP_VERSION_ID < 70000
...
PHP_MSHUTDOWN_FUNCTION(xdebug)
{
...

	{
		...
		if (XG(coverage_enable)) {
			...
			zend_set_user_opcode_handler(ZEND_BOOL_AND, NULL);
			zend_set_user_opcode_handler(ZEND_BOOL_OR, NULL);
#if PHP_VERSION_ID < 70000
