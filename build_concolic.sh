...
CFLAGS="${ZLOG_CFLAGS} ${XXHASH_CFLAGS} ${PROTOBUF_CLIENT_CFLAGS} ${MM_CFLAGS} ${TARGETS_CFLAGS} -DDISABLE_STRING_INTERNING -ggdb3"
LDFLAGS="${ZLOG_LDFLAGS} ${XXHASH_LDFLAGS} ${PROTOBUF_CLIENT_LDFLAGS} ${MM_LDFLAGS} ${TARGETS_LDFLAGS}"
LIBS="${ZLOG_LIBS} ${XXHASH_LIBS} ${PROTOBUF_CLIENT_LIBS} ${MM_LIBS} ${TARGETS_LIBS}"
