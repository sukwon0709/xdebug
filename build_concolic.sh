...
UTIL_PATH=/home/soh/git/uc-php/dependencies/uc-php-util
UTIL_CFLAGS="-I${UTIL_PATH}"
UTIL_LDFLAGS="-L${UTIL_PATH}"
UTIL_LIBS="${UTIL_LDFLAGS} -lucphp_util -lstdc++"

CFLAGS="${ZLOG_CFLAGS} ${XXHASH_CFLAGS} ${UCPHP_CFLAGS} ${PROTOBUF_CLIENT_CFLAGS} ${MM_CFLAGS} ${UTIL_CFLAGS} -DDISABLE_STRING_INTERNING -ggdb3"
LDFLAGS="${ZLOG_LDFLAGS} ${XXHASH_LDFLAGS} ${UCPHP_LDFLAGS} ${PROTOBUF_CLIENT_LDFLAGS} ${MM_LDFLAGS} ${UTIL_LDFLAGS}"
LIBS="${ZLOG_LIBS} ${XXHASH_LIBS} ${UCPHP_LIBS} ${PROTOBUF_CLIENT_LIBS} ${MM_LIBS} ${UTIL_LIBS}"
